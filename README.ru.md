...
| **Уровень 3** | LLM-арбитр (Cerebras Llama 3.3 70B) | ~200 мс | Асинхронный. Финальный вердикт для неоднозначных случаев |
...
**Уровень 3 — LLM-арбитр:** По умолчанию используется Cerebras (модель `llama-3.3-70b`, ~300 мс). Также поддерживаются OpenAI-совместимые API и Anthropic. Вызывается только при уровне уверенности между 0.5 и 0.7. Включает проверку соответствия намерений (intent alignment) для вызовов инструментов.
